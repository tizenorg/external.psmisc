...
void
printf_opt(const print_type type, const opt_type opts, const char *fmt, ...)
{
	va_list args;
	...
	for (nptr = names_head; nptr != NULL; nptr = nptr->next) {
		if (opts & OPT_SILENT) {
			...
			if (name_has_procs == 1 || opts & OPT_ALLFILES) {
				...
				printf_opt(fprintf_t, opts, "%s:", nptr->filename);
			...
			for (pptr = nptr->matched_procs; pptr != NULL;
				...
				if (pptr->proc_type == PTYPE_NORMAL)
					printf_opt(printf_t, opts, " %5d", pptr->pid);
				else
					printf_opt(printf_t, opts, "kernel");
				fflush(stdout);
				if (opts & OPT_VERBOSE) {
					switch (pptr->proc_type) {
					case PTYPE_KNFSD:
						printf_opt(fprintf_t, opts, " knfsd ");
						break;
					case PTYPE_MOUNT:
						printf_opt(fprintf_t, opts, " mount ");
						break;
					case PTYPE_SWAP:
						printf_opt(fprintf_t, opts, " swap  ");
						break;
					default:
						printf_opt(fprintf_t, opts, " %c%c%c%c%c ",
							pptr->access &
							ACCESS_FILE
							? (pptr->access &
							   ACCESS_FILEWR ? 'F' :
